...
public class ContentApiController {
    ...
    @GetMapping("/api/video/play/{contentId}")
    public ResponseEntity<?> getUserVideoInfo(@PathVariable Integer contentId){
        SessionUser sessionUser = (SessionUser) session.getAttribute("sessionUser");
        ContentResponse.PlayData respDTO = contentService.getUserVideoInfo(sessionUser.getId(), contentId);
        return ResponseEntity.ok(new ApiUtil<>(respDTO));
    }
}
